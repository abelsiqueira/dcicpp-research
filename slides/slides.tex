  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item Estender o Controle Dinâmico de Inviabilidade proposto por
        \citet*{bib:chico} para problemas de igualdades para tratar das
        desigualdades;
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item Extend the Dynamic Control of Infeasibility method proposed by
        \citet*{bib:chico} for equality problems to handle inequalities;
...
\myframe{
  \begin{center}
    \includegraphics[height=\textheight]{plots/contour.pdf}
  \end{center}
}
...
\myframe{
  \begin{center}
    ...
    \begin{axis}[colormap={bw}{gray(0cm)=(0.8);gray(1cm)=(1)}, domain=-1:3,
        domain y=-0.5:3.5, view={0}{90}, mark=none]
    \addplot3[contour gnuplot={number=40,labels=false}] {0.5*(x^2+y^2)};
    \end{axis}
    \begin{axis}[domain=-1:3, xmin=-1, xmax=3, ymin=-0.5, ymax=3.5]
      \addplot+[color=black, thick, mark=none] {x^2 + 1};
      \only<1-2>{
      \addplot+[color=red, dashed, mark=none] {x^2 + 1 + 1.13};
      \addplot+[color=red, dashed, mark=none] {x^2 + 1 - 1.13};
      \addplot+[color=red, dotted, thick, mark=none] {x^2 + 1 + 2*1.13};
      \addplot+[color=red, dotted, thick, mark=none] {x^2 + 1 - 2*1.13};
      }
      \only<3-6>{
      \addplot+[color=red, dashed, mark=none] {x^2 + 1 + 0.57};
      \addplot+[color=red, dashed, mark=none] {x^2 + 1 - 0.57};
      \addplot+[color=red, dotted, thick, mark=none] {x^2 + 1 + 2*0.57};
      \addplot+[color=red, dotted, thick, mark=none] {x^2 + 1 - 2*0.57};
      }
    \end{axis}
    \only<1>{
    \begin{axis}[xmin=-1, xmax=3, ymin=-0.5, ymax=3.5]
      \addplot[blue,mark=*,only marks] plot coordinates {
      (2,3) }
      node[pos=0, pin=300: {$x^0$}] {};
    \end{axis}
    }
    \only<2>{
    \begin{axis}[xmin=-1, xmax=3, ymin=-0.5, ymax=3.5]
      ...
      (2,3) (1.53,3.12) }
      node[pos=0, pin=300: {$x^0$}] {}
      node[pos=1, pin=300: {$x_c$}] {};
      \node (a) at (axis cs:2,3) {};
      \node (b) at (axis cs:1.53,3.12) {};
      \draw[blue,->] (a) -- (b);
    \end{axis}
    }
    \only<3>{
    ...
    \begin{axis}[xmin=-1, xmax=3, ymin=-0.5, ymax=3.5]
      ...
      node[pos=0, pin=300: {$x_c^1$}] {}
      node[pos=1, pin=30: {$x^+$}] {};
      \node (a) at (axis cs:1.53,3.12) {};
      \node (b) at (axis cs:0.43,-0.25) {};
      \draw[blue,->] (a) -- (b);
    \end{axis}
    }
    \only<5>{
    \begin{axis}[xmin=-1, xmax=3, ymin=-0.5, ymax=3.5]
      ...
      node[pos=1, pin=30: {$x^+$}] {};
      \node (a) at (axis cs:1.53,3.12) {};
      \node (b) at (axis cs:0.70,0.59) {};
      \draw[blue,->] (a) -- (b);
      \addplot[green,dashed] plot coordinates {
        (-2,0.59) (4,0.59)
      };
    \end{axis}
    }
    ...
    \begin{itemize}
      ...
      \item We used
        \begin{itemize}
          ...
          \item base\_matries (C++ wrapper for Cholesky);
          \item CUTEst \cite{bib:cutest} (testing);
...
\myframe {
  \ctr{ Experimentos Numéricos }
  \begin{itemize}
    \item 767 problemas do CUTEr (variáveis $\leq 100001$ e
      restrições $\leq 125025$).
    \item Problemas com variáveis fixas foram removidos.
  \end{itemize}
}
...
\myframe {
  \begin{center}
    \ctr{ \pten{Perfil de Desempenho}{Performance Profile} }
    \includegraphics{cuter_all.pdf}
  \end{center}
}
